...
function KPIRow() {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
      {cards.map((c) => <StatCard key={c.label} {...c} dark={false} />)}
    </div>
  )
}
    ...
    <div className="min-h-screen bg-surface-muted text-navy relative overflow-hidden flex flex-col justify-between">
      <div className="container-page py-8 space-y-6 relative z-10 flex-1">
        ...
        <FadeIn>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-2">
            <div>
              ...
              <h1 className="text-2xl md:text-3xl font-semibold text-navy tracking-tight">
            ...
            <div className="text-xs text-muted self-start sm:self-auto bg-white border border-surface-border px-3 py-1 rounded-none">
        ...
        <FadeIn delay={0.25}>
          <div className="grid md:grid-cols-2 gap-6">
            <CheckoutTrigger />
            <div className="bg-white border border-surface-border rounded-none p-5 flex flex-col justify-between">
              <div>
                <div className="flex items-center justify-between border-b border-surface-border pb-3 mb-4">
                  <h2 className="text-base font-semibold text-navy">Autonomous Pipeline Guide</h2>
                  <span className="text-2xs font-semibold text-primary bg-primary-50 px-2 py-0.5 rounded-none border border-primary-light">
                    Live System
                  </span>
                </div>
                <div className="space-y-3 text-xs text-muted leading-relaxed">
                  <div className="flex gap-2.5">
                    <span className="w-5 h-5 rounded-none bg-primary-50 text-primary border border-primary/20 font-semibold text-2xs flex items-center justify-center shrink-0 mt-0.5">1</span>
                    <div>
                      <strong className="text-navy font-semibold">Simulate a failure</strong> using the test cards on the left. The Razorpay checkout modal generates a mock transaction failure.
                    </div>
                  </div>
                  <div className="flex gap-2.5">
                    <span className="w-5 h-5 rounded-none bg-primary-50 text-primary border border-primary/20 font-semibold text-2xs flex items-center justify-center shrink-0 mt-0.5">2</span>
                    <div>
                      <strong className="text-navy font-semibold">Autonomous decisioning</strong> intercepts the webhook: checks bank degradation, evaluates the deterministic taxonomy safety gate, and computes causal uplift.
                    </div>
                  </div>
                  <div className="flex gap-2.5">
                    <span className="w-5 h-5 rounded-none bg-primary-50 text-primary border border-primary/20 font-semibold text-2xs flex items-center justify-center shrink-0 mt-0.5">3</span>
                    <div>
                      <strong className="text-navy font-semibold">Live Audit Trail</strong> records the chosen recovery action (retry, nudge, or human escalation) into PostgreSQL within 10 seconds.
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 pt-3 border-t border-surface-border text-2xs text-muted flex items-center justify-between">
                ...
                <span className="text-primary font-semibold">Use Copilot in the bottom-right →</span>
              </div>
            </div>
          </div>
        </FadeIn>
