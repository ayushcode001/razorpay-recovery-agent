...
function StatusBadge({ status }: { status: string }) {
  if (status === 'approved')
    return <span className="text-2xs font-semibold text-success bg-success-light border border-success/30 px-2 py-0.5 rounded-none uppercase">Approved</span>
  if (status === 'rejected')
    return <span className="text-2xs font-semibold text-muted bg-surface-subtle border border-surface-border px-2 py-0.5 rounded-none uppercase">Rejected</span>
  return <span className="text-2xs font-semibold text-warning-text bg-warning-light border border-warning/30 px-2 py-0.5 rounded-none uppercase">Pending</span>
}
    ...
    <div className="bg-white border border-surface-border rounded-lg shadow-sm overflow-hidden">
      <div className="px-5 py-4 border-b border-surface-border flex items-center justify-between">
        ...
        {proposals.filter(p => p.status === 'pending_human_approval').length > 0 && (
          <span className="text-2xs font-semibold text-danger bg-danger-light border border-danger/30 px-2 py-0.5 rounded-none">
