...
def print_drift_report(proposal: Dict[str, Any]):
    ...
    if proposal["drift_detected"]:
        ...
        print(f"   Current Net Value:         Rs.{proposal['net_value_current_threshold']:,} (Severe loss from retrying failing issuer)")
        print(f"   Proposed Net Value:        Rs.{proposal['net_value_proposed_threshold']:,} (Near-breakeven loss containment)")
        print(f"   Estimated Net INR Impact:  +Rs.{proposal['estimated_net_value_gain']:,} (Avoided loss / bleeding contained)")
